...
$context = array(
	...
	'isOffline'      => false,
	'suggestion'     => 0,
	'announcement'   => __( 'Core AI Living Block Map ready. Tap to add the blocks to the canvas.', 'core-ai-map' ),
...
<section <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> <?php echo wp_interactivity_data_wp_context( $context ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	<div class="core-ai-map__stage">
					...
					<div class="core-ai-map__actor-float">
						<?php if ( $is_skills ) : ?>
							...
							<button
								...
								type="button"
								aria-controls="<?php echo esc_attr( $instance_id . '-panel-skills' ); ?>"
								aria-expanded="false"
		...
		<nav class="core-ai-map__rail" aria-label="<?php esc_attr_e( 'Follow a story', 'core-ai-map' ); ?>" data-wp-bind--hidden="state.isRailHidden" hidden>
			<?php $step = 0; ?>
			<?php foreach ( $stories as $story_id => $story ) : ?>
				<?php ++$step; ?>
				<button
