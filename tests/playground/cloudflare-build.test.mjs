...
test( 'build emits the Pages rewrite for the literal remote.html endpoint', async ( t ) => {
	...
	assert.deepEqual(
		await readFile(
			join( outputDirectory, 'kiosk-blueprint', 'core-ai-map-3.2.6.zip' )
		),
		pluginContents
	);
	await assert.rejects(
	...
	assert.deepEqual( deploymentManifest.pluginArtifact, {
		path: 'kiosk-blueprint/core-ai-map-3.2.6.zip',
	...
	assert.equal( deploymentManifest.pluginVersion, '3.2.6' );
